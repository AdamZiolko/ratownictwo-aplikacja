...
export const useAudioOperations = () => {
  ...
  const [audioFileStatuses, setAudioFileStatuses] = useState<
    Record<string, AudioFileUiStatus>
  >({});
          ...
          FileSystem.getInfoAsync(localPath)
            .then(fileInfo => {
              setAudioFileStatuses(prevStatuses => ({
                ...
                [file.id]: {
                  ...prevStatuses[file.id],
                  isLocallyAvailable: fileInfo.exists,
                  isLoading: false,
                },
              }));
            })
  ...
  const handlePlayAudio = async (audio: AudioFile) => {
    ...
    if (
      playerState.playerModalVisible &&
      playerState.currentPlayingAudio?.id === audio.id
    ) {
      setPlayerState(prev => ({
        ...prev,
        playerModalVisible: false,
  ...
  const handleDownloadAudio = async (id: string, name: string) => {
    ...
    try {
      ...
      if (!downloadResponse.ok) {
        throw new Error(
          `Nie udało się pobrać pliku audio (status: ${downloadResponse.status})`
        );
      }
      ...
      if (Platform.OS === 'web') {
        ...
        Alert.alert(
          'Sukces',
          `Plik audio ${name} został pobrany i zapisany lokalnie.`
        );
        setAudioFileStatuses(prev => ({
          ...prev,
          [id]: {
      ...
      setAudioFileStatuses(prev => ({
        ...prev,
        [id]: { ...(prev[id] || {}), isLoading: false, error: error.message },
      }));
    }
  };
  ...
  useEffect(() => {
    if (
      playerState.playerModalVisible &&
      playerState.currentPlayingAudio &&
      !playerState.sound
    ) {
      (async () => {
        ...
        try {
          ...
          if (Platform.OS === 'web') {
            ...
            if (!streamResponse.ok)
              throw new Error('Failed to stream audio for web');
            ...
            if (fileInfo.exists) {
              ...
              if (!streamResponse.ok) {
                throw new Error(
                  `Failed to download audio: ${streamResponse.status} ${streamResponse.statusText}`
                );
              }
              const arrayBuffer = await streamResponse.arrayBuffer();
              const uint8Array = new Uint8Array(arrayBuffer);
          ...
          setAudioFileStatuses(prev => ({
            ...prev,
            [audio.id]: { ...(prev[audio.id] || {}), isLoading: false },
          }));
        } catch (error: any) {
          console.error('Błąd podczas ładowania audio w modalu:', error);
          Alert.alert(
            'Błąd',
            `Nie udało się załadować pliku audio: ${error.message}`
          );
          setAudioFileStatuses(prev => ({
            ...prev,
            [audio.id]: {
