...
const RhythmSelectionSlider = ({
  ...
  return (
    <View style={styles.sliderContainer}>
      <Text style={styles.sliderTitle}>Wybierz typ rytmu EKG</Text>      <View style={styles.container}>
        <IconButton
        icon="chevron-left"
        size={30}
...
const RhythmCard: React.FC<RhythmCardProps> = ({
  ...
  const animatedStyle = React.useMemo(() => {
    ...
    if (!scrollX || typeof index !== 'number') return baseStyle;
  ...
  useEffect(() => {
    ...
    const effectiveWidth = svgWidth;
    let path = "";

    for (let i = 0; i < pointsCount; i++) {
      const x = (i / pointsCount) * effectiveWidth;
