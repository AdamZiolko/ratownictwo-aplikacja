    ...
    useEffect(() => {
      const subscription = BluetoothSerial.onDeviceDisconnected(
        (disconnectedDevice) => {
          ...
          console.log(
            `Rozłączono urządzenie: ${disconnectedDevice.name || disconnectedDevice.address}`,
          );
        },
      );
    ...
    const checkDeviceProfiles = async (
      ...
      try {
        if (Platform.OS === "android") {
          const profiles =
            await nativeBluetoothModule.getDeviceProfiles(deviceAddress);
          return profiles || [];
        }
        return [];
      } catch (error) {
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            setError(
              `Połączono z ${device.name || device.address} (alternatywna metoda)`,
            );
            setShowConnectionStatus(true);
          } catch (altErr) {
