// app/components/ekg/Etco2Chart.tsx
...
const MAX_HISTORY = 300; // Liczba punktów do przechowywania
const CHART_WIDTH = 1000; // Szerokość wykresu w jednostkach SVG
...
const Etco2Chart: React.FC<Etco2ChartProps> = ({
  ...
  const updateChart = () => {
    ...
    for (let i = 0; i < dataRef.current.length; i++) {
      ...
      const y = height * (1 - Math.max(0, Math.min(100, dataRef.current[i]) / 100));
    ...
    >
      {width > 0 && (
        ...
        >
          ...
          {/* Siatka */}
          <G stroke={theme.dark ? "rgba(255,0,0,0.1)" : "rgba(255,255,255,0.1)"}>
